...
public interface DrugDao {
    ...
    @Transaction
    @Query("SELECT * FROM drugs " + "ORDER BY title ASC")
    ...
    @Query(
            "SELECT * FROM drugs "
                    + "WHERE stockAmount > 0 AND isFavorite = 1 AND drugs.title LIKE '%' || "
                    + ":searchTerm || "
                    + "'%' "
                    + "ORDER BY title ASC")
    ...
    @Query(
            "SELECT * FROM drugs "
                    + "WHERE stockAmount > 0 AND drugs.drugTypeId IN (:drugTypeIds) AND drugs"
                    + ".title LIKE '%' "
                    + "|| :searchTerm || '%' "
                    + "ORDER BY title ASC")
    ...
    @Query(
            "SELECT * FROM drugs "
                    + "WHERE stockAmount > 0 AND isFavorite = 1 AND drugs.drugTypeId IN "
                    + "(:drugTypes) AND "
                    + "drugs.title LIKE '%' || :searchTerm || '%' "
                    + "ORDER BY title ASC")
    LiveData<List<DrugWithUnitAndDrugTypeAndSubstance>> getOnStockFavoriteDrugsByDrugTypes(
            List<Integer> drugTypes, String searchTerm);

    @Transaction
    @Query(
            "UPDATE drugs SET isFavorite = ((isFavorite | 1) - (isFavorite & 1)) WHERE drugId = "
                    + ":drugId")
